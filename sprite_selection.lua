...
moveGalleryLeftButton:setReferencePoint( display.TopLeftReferencePoint )
...
moveGalleryRightButton:setReferencePoint( display.TopLeftReferencePoint )
...
function  SelectSprite( event )
		...
		if i~=-1 then
			...
			local s=sprite_collection.SpriteColection[i]
			CurrentSpriteImage=display.newImage( s.image )
			CurrentSpriteImage:setReferencePoint( display.TopLeftReferencePoint )
...
function drawSpriteGallery(self)
	...
	for i,line in ipairs(sprite_collection.SpriteColection) do
		...
		SelectedSpriteObject=display.newImageRect( s.image ,s.width,s.height)
		...
		SelectedSpriteObject:setReferencePoint( display.TopLeftReferencePoint )
